...
**Na data do piloto, recomendou-se o ambiente controlado para a fase Petstore planejada.** A recomendação se baseou no ciclo funcional observado equivalente ao público, no contrato servido idêntico à cópia normativa, na limpeza confirmada e, sobretudo, na possibilidade de isolar dados e evitar interferência de terceiros. Antes do congelamento, deveriam ser aceitos e documentados o uso da imagem fixada por digest, a emulação `linux/amd64` neste host e a exclusão de autenticação como oráculo. Naquele momento, a fase Petstore ainda não havia sido executada.
